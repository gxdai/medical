...
def main(args):
    ...
    train_cls= generateList('train.txt')
    train_filenames, train_labels = generateData(train_cls, step=args.dn_train, dpFlag=True, targetNum=args.targetNum)

    print(len(train_filenames))
    print(len(train_labels))
    # Get testing data
    test_cls = generateList('test.txt')
    if args.dn_test == 0:
        val_filenames, val_labels = generateData(test_cls, step=None, dpFlag=False)
    else:
        val_filenames, val_labels = generateData(test_cls, step=args.dn_test, dpFlag=False)
    ...
    with graph.as_default():
        ...
        fc8_optimizer = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=0.9)
    ...
    with tf.Session(graph=graph, config=config) as sess:
        ...
        for epoch in range(args.num_epochs1):
            ...
            while True:
            	...
            	counter += 1
            	if counter % 100 == 0:
            		print("{}, Iter: {:<6d}, Loss: {:10.5f}".format(datetime.now(), counter, epoch_loss))
            		epoch_loss = 0.
            	if counter % 2000 == 0:
            		val_acc, val_acc_weighted, val_acc_for_each_class = check_accuracy(sess, correct_prediction, labels, prediction, is_training, val_init_op)
            		print('Val, overall accuracy: {:6.5f}, weighted accuracy {:6.5f} >>>>\n'.format(val_acc, val_acc_weighted))
                ...
                try:
                    if epoch < 5:
                        _, loss_value = sess.run([fc8_train_op, loss], {is_training: True, learning_rate: args.learning_rate1})
                    else:
                        _, loss_value = sess.run([fc8_train_op, loss], {is_training: True, learning_rate: args.learning_rate2})
